...
enum StatusIconRenderer {
    private static let iconColor = NSColor.black

    static func image(for style: StatusIconStyle, size: CGFloat = 16) -> NSImage? {
        guard let image = StatusIconTemplateDrawer.image(
            for: style,
            size: size,
            tintColor: iconColor
        ) else {
...
private enum StatusIconTemplateDrawer {
    static func image(for style: StatusIconStyle, size: CGFloat, tintColor: NSColor) -> NSImage? {
        ...
        let iconRect = CGRect(origin: .zero, size: NSSize(width: size, height: size)).insetBy(dx: 0.5, dy: 0.5)
        context.saveGState()
